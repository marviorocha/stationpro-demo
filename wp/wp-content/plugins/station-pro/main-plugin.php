...
if (! class_exists('Redux_Framework_Plugin', false)) {
    ...
    class Redux_Framework_Plugin
    {
         ...
         * @since       3.0.0
         */
        protected $plugin_slug = 'station-pro';
        ...
        public static function instance(): ?Redux_Framework_Plugin
        {
            ...
            if (function_exists('get_plugin_data') && file_exists($path)) {
                ...
                if (isset($data) && isset($data['Version']) && '' !== $data['Version']) {
                    $res = version_compare($data['Version'], '3', '<');
                }
        ...
        public function get_redux_options()
        {
            ...
            if (is_multisite()) {
                ...
                foreach ($plugins as $file => $plugin) {
                    if (strpos($file, 'stationpro.php') !== false) {
        ...
        public function load_sample_config()
        {
            ...
            // }
            require_once dirname(__FILE__) . '/app/default-config.php';
        }
        ...
        public function load_first()
        {
            if (! class_exists('Redux_Functions_Ex')) {
                require_once dirname(__FILE__) . '/core/inc/classes/class-redux-functions-ex.php';
            }
        ...
        public function options_toggle_check()
        {
            if (isset($_GET['nonce']) && wp_verify_nonce(sanitize_key($_GET['nonce']), 'redux_framework_demo')) {
                if (isset($_GET['redux-framework-plugin']) && 'demo' === $_GET['redux-framework-plugin']) {
                    $url = admin_url(add_query_arg(array( 'page' => 'station-pro' ), 'options-general.php'));
        ...
        {
            if (basename(plugin_dir_path(__FILE__)) . '/stationpro.php' !== $file) {
                return $links;
            }
        ...
        {
            // if ( strpos( $file, 'stationpro.php' ) !== false && is_plugin_active( $file ) ) {
            //  $links[] = '<a href="' . esc_url( admin_url( add_query_arg( array( 'page' => 'station-pro' ), 'options-general.php' ) ) ) . '">' . esc_html__( 'What is this?', 'stationpro' ) . '</a>';
            // }
