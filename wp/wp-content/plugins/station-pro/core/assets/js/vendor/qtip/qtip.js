    ...
    (function ($) {
        ...
        PROTOTYPE.render = function (show) {
            if (this.rendered || this.destroyed) {
                return this; } // If tooltip has already been rendered, exit
        ...
        PROTOTYPE.toggle = function (state, event) {
            ...
            if (!this.rendered) {
                return state ? this.render(1) : this; } else if (this.destroyed || this.disabled) {
                return this; }
            ...
            // Execute state specific properties
                if (state) {
                    ...
                    if (typeof contentOptions.text === 'function') {
                        this._updateContent(contentOptions.text, false); }
                    if (typeof contentOptions.title === 'function') {
        ...
        function hideMethod(event)
        {
            ...
            if (this !== relatedTarget[0]
                ...
                || this.options.hide.fixed && (           (/mouse(out|leave|move)/).test(event.type) && (ontoTooltip || ontoTarget))
            ) {
                /* eslint-disable no-empty */
                try {
        ...
        function init(elem, id, opts)
        {
            ...
            /* eslint-disable no-empty */
            try {
                html5 = typeof html5 === 'string' ? JSON.parse(html5) : html5; } catch (e) {
                }
            /* eslint-enable no-empty */
            ...
            $.cleanData = function ( elems ) {
                for (var i = 0, elem; (elem = $(elems[i])).length; i++) {
                    if (elem.attr(ATTR_HAS)) {
                             /* eslint-disable no-empty */
                        try {
                            elem.triggerHandler('removeqtip'); } catch ( e ) {
                            }
                             /* eslint-enable no-empty */
                    }
                }
            ...
            Tip.prototype,
            {
                ...
                update: function (corner, position) {
                    ...
                    // Otherwise inherit mimic properties from the corner object as necessary
                        ...
                        if (mimic.x === 'inherit') {
                            mimic.x = corner.x; } else if (mimic.y === 'inherit') {
                            mimic.y = corner.y; } else if (mimic.x === mimic.y) {
        ...
        MODAL.sanitize = function (opts) {
            if (opts.show) {
                if (typeof opts.show.modal !== 'object') {
                    opts.show.modal = { on: !!opts.show.modal }; } else if (typeof opts.show.modal.on === 'undefined') {
            ...
            polygon: function (baseCoords, corner) {
                ...
                if (corner.abbrev() === 'c') {
                    ...
                    while (newWidth > 0 && newHeight > 0 && compareX > 0 && compareY > 0) {
                        ...
                        if (corner.x === LEFT) {
                            compareX = newWidth; } else if (corner.x === RIGHT) {
                            compareX = result.width - newWidth; } else {
                                compareX += Math.floor(newWidth / 2); }

                            if (corner.y === TOP) {
                                compareY = newHeight; } else if (corner.y === BOTTOM) {
                                                        compareY = result.height - newHeight; } else {
